...
import { LayoutDashboard, Building2, Link2, Wallet2, X } from 'lucide-react';

export const PARTNER_TABS = [
    { key: 'dashboard', label: 'Дашборд', icon: LayoutDashboard },
    { key: 'advertisers', label: 'Мои Бренды', icon: Building2 },
    { key: 'promo', label: 'Промо и Ссылки', icon: Link2 },
    { key: 'wallet', label: 'Кошелек и Выплаты', icon: Wallet2 },
];

/**
 * Partner Cabinet's side nav — same persistent-column-on-desktop / slide-over-drawer-on-mobile
 * shell as AdvertiserSidebar, just with the partner's own section list.
 */
export default function PartnerSidebar({ activePage, onChangePage, open, onClose }) {
    const nav = (
        <nav className="space-y-1">
            {PARTNER_TABS.map(({ key, label, icon: Icon }) => {
