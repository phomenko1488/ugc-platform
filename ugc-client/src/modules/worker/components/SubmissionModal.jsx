...
/**
 * Submission form for a chosen offer — platform, source URL, declared views, analytics
 * screenshot (Module 4's FileUploader), and a live estimated-hold calculator:
 * hold = declaredViews * workerCpmRate / 1_000_000.
 */
export default function SubmissionModal({ worker, offer, onClose, onSubmitted }) {
    ...
    const [declaredViews, setDeclaredViews] = useState(offer.minViewsThreshold || 50000);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!Number(declaredViews) || Number(declaredViews) <= 0) {
            setError('Укажите корректное количество просмотров');
            return;
        }
        ...
        try {
            ...
            await api.submitVideo({
                ...
                declaredViews: Number(declaredViews),
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center">
            <div className="bg-brand-card border border-brand-border rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md max-h-[92vh] overflow-y-auto shadow-2xl">
                ...
                <div className="p-5 space-y-4">
                    <div className="p-3 rounded-xl bg-brand-accent/5 border border-brand-accent/20 flex items-start gap-2.5 text-xs text-brand-accent">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-slate-300 mb-1.5">Текущие просмотры</label>
                            <input
                                type="number"
                                min={1}
                                step={1000}
